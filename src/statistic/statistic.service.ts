...
export class StatisticService {
    ...
    async getMainStatistics(storeId: string) {
        const totalReveune = await this.calculateTotalRevenue(storeId)

        const prosuctsCount = await this.countProducts(storeId)
        const categoriesCount = await this.countCategories(storeId)

        const averageRating = await this.calculateAverageRating(storeId)

        return [
            { id: 1, name: "Выручка", value: totalReveune },
            { id: 2, name: "Товары", value: prosuctsCount },
    ...
    private async countProducts(storeId: string) {
        const prosuctsCount = await this.prisma.product.count({
            where: { storeId }
        })
        return prosuctsCount
    }

    private async countCategories(storeId: string) {
        const categoriesCount = await this.prisma.product.count({
            where: { storeId }
        })
